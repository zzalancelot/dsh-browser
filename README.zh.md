...
浏览器操作以文本为主：页面会转换为结构化文本和带编号的交互元素清单，模型通过编号定位元素。当文本清单无法描述页面（canvas、验证码、图形或抓取失败）时，在设置中开启**截图增强模式**后，可用 `browser_screenshot` 截取受控标签页作为视觉兜底。dsh 0.1.5 的多模态对话仍走独立通道——宿主声明图片能力时，侧栏也可接收用户主动添加的 PNG、JPEG、WebP 和 GIF。
...
- **纯文本优先的页面接口**：编号控件、跨快照稳定 ID、delta 更新和敏感值掩码，使模型在常见场景下无需截图也能操作页面；开启截图增强模式后，抓取失败时可用 `browser_screenshot` 作为视觉兜底；用户主动添加的对话图片走 dsh 独立的多模态消息通道。
...
- 单活动连接；浏览器页面管线以文本为主，受控标签页在文本抓取失败时可经审批调用 `browser_screenshot`（需在设置中开启截图增强模式，默认关闭）；用户主动添加的对话图片交给 dsh 持久附件服务，密码和卡号值永不回传。
