...
**两条明确分离的通道**：浏览器页面默认以结构化文本呈现（带编号的交互元素清单）；当文本无法描述页面时，`browser_screenshot` 会截取受控标签页作为视觉兜底。另一方面，dsh 0.1.2+ 宿主可声明多模态图片限制，侧栏据此接收用户主动添加的 PNG、JPEG、WebP 和 GIF，并渲染会话中的持久图片附件。
...
| 截取页面 | `browser_screenshot` | 文本快照无法描述页面时的视觉兜底（canvas/验证码/抓取失败） |
...
- **content script**（`src/content/`）：纯文本快照（可读性主文 + 编号交互清单 + 表单字段）、**稳定编号**（WeakMap；可选写入 `data-dsh-el`）、delta 变化、点击/输入/按键/滚动/导航动作、敏感字段掩码，以及带防抖的划选监听——只有侧栏打开且页面共享不是「关闭」时才会启用。
...
- **稳定编号**：元素编号跨快照保持在 WeakMap 中（设置中开启「写入观察属性」时才会镜像到 `data-dsh-el`，默认关闭），模型可以说"点 7 号"；页面大改时显式提示"编号已重排"。
...
- 页面操作用合成 `dispatchEvent`，因此 `event.isTrusted` 恒为 `false`，内容脚本无法伪造；只有 CDP / `chrome.debugger` 可信输入能改变这一点（额外权限 + 调试横幅）。**隐身输入**可减少机械指纹（默认不写持久 `data-dsh-el`；开启时有指针轨迹与最小滚动），但不承诺规避站点反自动化。
