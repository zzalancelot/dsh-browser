...
describe('browser locale selection', () => {
  ...
  it('provides localized panel and prompt copy in both languages', () => {
    ...
    expect(PANEL_COPY.en.settings.stealthMode).toBe('Stealth input')
    expect(PANEL_COPY.en.settings.writeObservationAttribute).toBe('Write observation attributes')
    ...
    expect(PANEL_COPY.zh.settings.stealthMode).toBe('隐身输入')
    expect(PANEL_COPY.zh.settings.writeObservationAttribute).toBe('写入观察属性')
  })
})
